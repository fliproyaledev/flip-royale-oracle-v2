...
export type Token = {
    ...
    symbol: string
    name: string
    about?: string // Mapped from TYPE field
...
function rowToToken(row: RawRow): Token {
    ...
    // Link temizliği
    // Frontend'deki gibi güçlü bir temizleyici kullanıyoruz
    const rawLink = String(row['GECKO TERMINAL POOL LINK'] || row['dexscreenerPair'] || '').trim()
    // Önce basit temizlik
    let cleanPair = cleanAddress(rawLink)
    // Eğer basit temizlik yetmediyse ve URL ise, içinden adresi çekmeyi dene
    ...
    return {
        ...
        name: name,
        about: String(row['TYPE'] || '').trim(), // TYPE Mapping Here
...
const seedTokens: Token[] = [
    {
        ...
        dexscreenerPair: '0x3f0296bf652e19bca772ec3df08b32732f93014a' // Virtual Contract/Pool
    },
]
